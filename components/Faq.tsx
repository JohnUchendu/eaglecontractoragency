      ...
      <Accordion type="single" collapsible> 
        {faqs.map((faq, index) => (
          <AccordionItem key={index} value={`faq-${index}`}>
            <AccordionTrigger>{faq.question}</AccordionTrigger> {/* ✅ Correct way to show the question */}
            <AccordionContent>
              <p className="text-gray-700 dark:text-gray-300">{faq.answer}</p> {/* ✅ Correct way to show the answer */}
            </AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
